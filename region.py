...
if df_region.empty:
    ...
    fig2.add_trace(
        go.Scatter(
            x=daily["period"], y=daily["total_demand"],
    ...
    if not high_days.empty:
        fig2.add_trace(go.Scatter(
            x=high_days["period"], y=high_days["total_demand"],
            mode="markers", name="High anomaly",
            marker=dict(color="red", size=10, symbol="triangle-up"),
        ))
    if not low_days.empty:
        fig2.add_trace(go.Scatter(
            x=low_days["period"], y=low_days["total_demand"],
            mode="markers", name="Low anomaly",
    ...
    # Day-over-day change
    fig3 = px.bar(
        daily, x="period", y="demand_pct_change",
...
if available_dates:
    selected_date = st.selectbox("Select a date to compare all regions", options=available_dates, index=len(available_dates) // 2)
    day_df = df[df["period"].dt.strftime("%Y-%m-%d") == selected_date]
    day_agg = day_df.groupby("respondent")[ycol].sum().reset_index().sort_values(ycol, ascending=True)
